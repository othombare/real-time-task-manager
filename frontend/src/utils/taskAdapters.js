...
const getTaskComparableOrder = (task = null) => {
  ...
  const fallbackDate = new Date(task?.createdAt || task?.updatedAt || 0).getTime();
  return Number.isFinite(fallbackDate) ? fallbackDate : 0;
};

const compareTaskOrder = (left, right) => {
  const orderDifference = getTaskComparableOrder(left) - getTaskComparableOrder(right);
...
export const getTaskSortOrderBetween = (previousTask, nextTask) => {
  const previousOrder = Number(previousTask?.sortOrder);
  const nextOrder = Number(nextTask?.sortOrder);
...
export const mapColumnToApiTaskStatus = (status) =>
  columnToApiStatusMap[String(status || "").trim()] || "todo";
...
export const normalizeApiTask = (task, project = null) => {
  ...
  return {
    ...
    projectSlug,
    sortOrder: Number.isFinite(Number(task?.sortOrder)) ? Number(task.sortOrder) : null,
    supportsTaskDetailsEditing: true,
  };
};

export const buildTaskDescription = (description = "", notes = "") =>
...
export const buildTaskCreatePayload = (task = {}, projectId) => {
  ...
  return {
    ...
    sortOrder: Number.isFinite(Number(task?.sortOrder)) ? Number(task.sortOrder) : undefined,
  };
};

export const buildTaskUpdatePayload = (updates = {}) => {
  const payload = {};
  ...
  if (updates.sortOrder !== undefined) {
    const numericSortOrder = Number(updates.sortOrder);

    if (Number.isFinite(numericSortOrder)) {
      payload.sortOrder = numericSortOrder;
    }
  }
...
export const buildProjectBoardFromTasks = (tasks = []) => {
  ...
  return groupedBoard.map((column) => ({
    ...column,
    tasks: [...column.tasks].sort(compareTaskOrder),
  }));
};
